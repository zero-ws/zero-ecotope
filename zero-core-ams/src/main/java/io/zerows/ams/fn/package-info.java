 ...
 * 注意，扩展定义中，如果是 RuntimeException 没有必要定义函数式接口，JVM 语言规范本身就支持，并且 running 异常
 ...
 * 三种异常的前缀：
 * - checked: 来自于JVM级别的异常，异常顶层类 Throwable（范围最大）
 * - fail: 来自于JVM级别的检查异常，异常顶层类 Exception（范围中等）
 * - bug: 来自于程序级别的异常，异常顶层类 ProgramException（范围最小）
 * =====================
 * 根据实际定义可以知道：
 * 1. bug 前缀方法是 ProgramException 引起的，它和 checked / fail 的区别在于
 * -- bug 类的方法需要抛出检查异常 ProgramException
 * -- fail / checked 类的方法只是需要在后台打印堆栈信息，不需要抛出异常
 * 2. fail 和 checked 类的方法一个是用于处理 Exception，一个用于处理 Throwable，二者在定义时
