...
(
    32
) COMMENT '「pNavTheme」- navTheme, 风格处理，对应 light / realdark',
    `P_COLOR_PRIMARY` VARCHAR
(
    20
) COMMENT '「pColorPrimary」- colorPrimary，主色调',
    `P_LAYOUT` VARCHAR
(
    12
) COMMENT '「pLayout」- 布局类型：top, menu, mix',
    `P_CONTENT_WIDTH` VARCHAR
(
    12
) COMMENT '「pContentWidth」- 两种',
...
(
    36
) COMMENT '「myBag」- 对应 MY_BAG 设置，每个BAG有对应设置信息',
    `OWNER` VARCHAR
(
    36
