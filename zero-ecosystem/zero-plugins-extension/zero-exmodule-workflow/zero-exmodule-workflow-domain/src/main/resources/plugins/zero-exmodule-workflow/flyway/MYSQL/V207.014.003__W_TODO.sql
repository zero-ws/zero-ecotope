...
(
    128
) COMMENT '「serial」- 待办编号，使用 X_NUMBER 生成',
    `NAME` VARCHAR
(
    255
) COMMENT '「name」- 待办名称（标题）',
    `CODE` VARCHAR
(
    128
) COMMENT '「code」- 待办系统码，使用流程时候关联流程的任务ID',
    `ICON` VARCHAR
(
    255
) COMMENT '「icon」- 待办显示的图标',
    -- 待办相关内容
    /*
     * 待办状态清单：
     * PENDING：新生成的待办，等待办理
...
(
    36
) COMMENT '「parentId」- 待办支持父子集结构，父待办执行时候子待办同样执行',
    /*
     * 1:1,
     * 1:N,
     * N:1,
...
(
    255
) COMMENT '「serialFork」- 生成序号的分支序号',

    -- 主单ID（TraceId）
    `TRACE_ID` VARCHAR
(
    36
) COMMENT '「traceId」- 同一个流程的待办执行分组',
    `TRACE_ORDER` INTEGER COMMENT '「traceOrder」- 待办的处理顺序',

    `TASK_ID` VARCHAR
(
    36
) COMMENT '「taskId」- 和待办绑定的taskId（任务）',
    `TASK_KEY` VARCHAR
(
    255
...
(
    36
) COMMENT '「toLocation」- 指定地址区域',
    `TO_GROUP` VARCHAR
(
    36
) COMMENT '「toGroup」- 指定用户组',
    `TO_TEAM` VARCHAR
(
    36
) COMMENT '「toTeam」- 指定业务组',
    `TO_ROLE` VARCHAR
(
    36
) COMMENT '「toRole」- 待办角色（集体）',
    /*
     * 目前在使用的指定待办人
