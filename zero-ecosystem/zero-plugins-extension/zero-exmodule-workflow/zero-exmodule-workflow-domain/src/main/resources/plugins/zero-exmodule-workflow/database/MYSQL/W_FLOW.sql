...
(
    255
) COMMENT '「code」- 流程定义编号（系统可用）',
    `TYPE` VARCHAR
(
    36
) COMMENT '「type」- 流程类型，对接 zero.workflow.cat的X_TABULAR',

    -- 关联Camunda读取流程图
    `DEFINITION_KEY` VARCHAR
(
    128
) COMMENT '「definitionKey」- 定义ID（读取流程图所需）, getProcessDefinitionId',

    -- 读取流程授权专用组件以及配置
    `AUTHORIZED_COMPONENT` VARCHAR
(
    255
) COMMENT '「authorizedComponent」- 流程授权组件',
    `AUTHORIZED_CONFIG` LONGTEXT COMMENT '「authorizedConfig」- 流程授权配置',

    -- Todo生成组件
    `GENERATE_COMPONENT` VARCHAR
(
    255
) COMMENT '「generateComponent」- Todo生成组件',
