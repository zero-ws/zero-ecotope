...
(
    255
) COMMENT '「name」- 文件名（带扩展名）',
    `EXTENSION` VARCHAR
(
    10
) COMMENT '「extension」- 文件扩展名',
    -- 文件管理专用
    `TYPE` VARCHAR
(
    128
) COMMENT '「type」- 文件类型，直接关联zero.file.tree类型',

    `MIME` VARCHAR
(
    128
) COMMENT '「mime」- 该文件的MIME类型',
    `SIZE` INTEGER COMMENT '「size」- 该文件的尺寸',

    `STATUS` VARCHAR
(
    12
) COMMENT '「status」- 状态，PROGRESS / SUCCESS',
    /*
     * 附件存储方式说明
     * - storeWay
...
(
    36
) COMMENT '「directoryId」- 文件存储所属目录',
    `STORE_WAY` VARCHAR
(
    255
) COMMENT '「storeWay」- 存储方式，BLOB / FILE / REMOTE',
    `STORE_PATH` VARCHAR
(
    1024
) COMMENT '「storePath」- 远程存储的目录信息（显示专用，去服务器和协议部分）',
    `STORE_URI` VARCHAR
(
    1024
) COMMENT '「storeUri」- 远程存储的目录URI部分',
...
(
    36
) COMMENT '「modelCategory」- 如果一个模型记录包含多种附件，则需要设置模型相关字段，等价于 field',

    -- 原始文件相关信息
    `FILE_NAME` VARCHAR
(
    255
) COMMENT '「fileName」- 原始文件名（不带扩展名）',
    `FILE_KEY` VARCHAR
(
    255
) COMMENT '「fileKey」- TPL模式中的文件唯一的key（全局唯一）',
    `FILE_URL` VARCHAR
(
    255
) COMMENT '「fileUrl」- 该文件的下载链接（全局唯一）',
    `FILE_PATH` VARCHAR
(
    255
) COMMENT '「filePath」- 该文件的存储地址，FILE时使用', -- Vert.x 中的地址规范
