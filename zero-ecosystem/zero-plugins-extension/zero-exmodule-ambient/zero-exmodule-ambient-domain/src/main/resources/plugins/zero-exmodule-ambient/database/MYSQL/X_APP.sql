...
(
    128
) COMMENT '「appSecret」- 专用密钥',
    `APP_KEY` VARCHAR
(
    128
) COMMENT '「appKey」- 应用程序专用唯一hashKey',

    -- 常用属性
    `TITLE` VARCHAR
(
    64
) COMMENT '「title」- 应用程序标题',
...
(
    255
) COMMENT '「copyRight」- CopyRight版权信息',
    `EMAIL` VARCHAR
(
    255
) COMMENT '「email」- 应用Email信息',
...
(
    255
) COMMENT '「domain」- 应用程序所在域',
    `PORT` INTEGER COMMENT '「port」- 应用程序端口号，和SOURCE的端口号区别开',
    `CONTEXT` VARCHAR
(
    255
) COMMENT '「configure」- 应用程序路径',
    `URL_LOGIN` VARCHAR
(
    255
) COMMENT '「urlLogin」— 应用程序入口页面（登录页）',
    `URL_ADMIN` VARCHAR
(
    255
) COMMENT '「urlAdmin」- 应用程序内置主页（带安全）',
...
(
    255
) COMMENT '「endpoint」- 后端API的根路径，启动时需要',
    `ENTRY` VARCHAR
(
    255
) COMMENT '「entry」- App 关联的入口菜单',
