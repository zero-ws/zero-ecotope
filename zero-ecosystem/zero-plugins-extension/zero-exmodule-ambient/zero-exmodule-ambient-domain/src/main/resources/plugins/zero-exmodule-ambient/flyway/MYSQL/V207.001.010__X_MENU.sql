...
CREATE TABLE IF NOT EXISTS `X_MENU` (
    ...
    `LEVEL`       BIGINT        DEFAULT NULL COMMENT '「level」- 菜单层级',
    `NAME`        VARCHAR(255)  COLLATE utf8mb4_bin DEFAULT NULL COMMENT '「name」- 名称',
    `ORDER`       BIGINT        DEFAULT NULL COMMENT '「order」- 顺序',
    `PARENT_ID`   VARCHAR(36)   COLLATE utf8mb4_bin DEFAULT NULL COMMENT '「parentId」- 父节点',
    `TEXT`        VARCHAR(255)  COLLATE utf8mb4_bin DEFAULT NULL COMMENT '「text」- 菜单文字',
    ...
    UNIQUE KEY `UK_X_MENU_NAME_APP_ID` (`NAME`, `APP_ID`) USING BTREE,
    KEY `IDX_X_MENU_APP_ID` (`APP_ID`) USING BTREE,
    KEY `IDX_X_MENU_APP_ID_SIGMA` (`APP_ID`, `SIGMA`) USING BTREE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_bin COMMENT='X_MENU';
