...
import io.zerows.extension.module.mbsecore.domain.tables.daos.MJoinDao;
...
class AoRefineModel implements AoRefine {
    ...
    private Future<JsonArray> saveAttrAsync(final Model model) {
        ...
        // 2. 从数据库中读取原始属性
        final ADB jooqAttr = DB.on(MAttributeDao.class);
    ...
    private Future<JsonArray> saveJoinAsync(final Model model) {
        ...
        model.dbJoins().stream().map(fnQuery)
            .forEach(each -> criteria.put("$" + each.hashCode(), each));
        // 2. 从数据库中读取初始属性
        final ADB jooqJoin = DB.on(MJoinDao.class);
