...
(
    255
) COMMENT '「identifier」- 当前模型全局唯一ID',
    `NAMESPACE` VARCHAR
(
    255
) COMMENT '「namespace」- 当前模型使用的名空间',
    `NAME` VARCHAR
(
    255
) COMMENT '「name」- 当前模型的名称',
    `ALIAS` VARCHAR
(
    255
) COMMENT '「alias」- 模型别名（业务名）',
    `TYPE` VARCHAR
(
    10
) COMMENT '「type」- 当前模型的类型信息',
...
(
    255
) COMMENT '「categoryTree」- 当前模型所属的类型树',
    `CATEGORY_ID` VARCHAR
(
    36
) COMMENT '「categoryId」- 关联的类型的ID',
    ...
    `SPIDER_COMPONENT` VARCHAR
(
    255
) COMMENT '「spiderComponent」- 在主图格式上存在的插件信息，用于处理主图格式的内容',
