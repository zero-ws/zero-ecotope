...
(
    36
) COMMENT '「code」- neo4j 中的图的 label，符合 neo4j的图ID规范（使用一定命名规则）',
    `MODE` VARCHAR
(
    20
) COMMENT '「mode」- 图模型 FLOW / TOPOLOGY / MIND / TREE',
    `TYPE` VARCHAR
(
    36
) COMMENT '「type」- 图类型 CIRCLE / DEFINE / INSTANCE 圈子、定义、实例，可扩展 tabular',
...
(
    36
) COMMENT '「ownerId」- 图的拥有者，可管理该图信息的人员ID',
    `UI` TEXT COMMENT '「ui」- ui配置专用',
    `GRAPHIC_ID` VARCHAR
(
    36
) COMMENT '「graphicId」- 父图ID（当前图是父图的子图，独立管理，创建时需要）',
