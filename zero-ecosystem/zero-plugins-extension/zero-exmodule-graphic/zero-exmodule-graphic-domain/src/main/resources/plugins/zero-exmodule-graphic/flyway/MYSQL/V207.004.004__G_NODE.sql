...
(
    18,
    6
) COMMENT '「x」 - 当前节点在图上的x坐标',
    `Y` DECIMAL
(
    18,
    6
) COMMENT '「y」 - 当前节点在图上的y坐标',
...
(
    36
) COMMENT '「graphicId」 - 它所关联的图实例ID',
    `RECORD_DATA` TEXT COMMENT '「recordData」 - 该节点存储的数据信息',
    `RECORD_KEY` VARCHAR
(
    36
) COMMENT '「recordKey」 - 记录主键',
    `RECORD_COMPONENT` VARCHAR
(
    255
) COMMENT '「recordComponent」 - 记录处理组件',
    `RECORD_CLASS` VARCHAR
(
    255
) COMMENT '「recordClass」 - 记录绑定Pojo类型',
