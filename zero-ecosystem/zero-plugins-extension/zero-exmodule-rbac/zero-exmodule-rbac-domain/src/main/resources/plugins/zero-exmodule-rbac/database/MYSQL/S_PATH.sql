...
(
    36
) COMMENT '「parentId」- 区域模式下的父ID，系统内部读取',
...
(
    255
) COMMENT '「runComponent」- HValve执行组件，组件内置处理 dm / ui 两部分内容',
    `RUN_TYPE` VARCHAR
(
    5
) COMMENT '「runType」- 视图管理类型（查询用）',
...
(
    ...
    `DM_COMPONENT` VARCHAR
(
    255
) COMMENT '「dmComponent」- 必须绑定组专用Dao组件',
