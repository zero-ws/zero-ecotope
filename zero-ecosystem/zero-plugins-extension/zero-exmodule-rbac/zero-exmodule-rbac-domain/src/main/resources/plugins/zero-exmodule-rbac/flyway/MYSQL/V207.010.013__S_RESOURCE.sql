...
(
    255
) COMMENT '「identifier」- 当前资源所属的Model的标识',
...
(
    32
) COMMENT '「modeRole」- 该资源查找角色的模式',
    `MODE_GROUP` VARCHAR
(
    32
) COMMENT '「modeGroup」- 该资源查找组的模式',
    `MODE_TREE` VARCHAR
(
    32
) COMMENT '「modeTree」- 该资源处理树（用户组）的模式',
    ...
    `SEEK_COMPONENT` VARCHAR
(
    255
) COMMENT '「seekComponent」- 访问者组件',
