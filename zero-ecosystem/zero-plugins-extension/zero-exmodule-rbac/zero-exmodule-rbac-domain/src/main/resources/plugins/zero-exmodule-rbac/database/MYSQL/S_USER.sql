...
(
    255
) COMMENT '「username」- 用户登录账号',
    `REALNAME` VARCHAR
(
    255
) COMMENT '「realname」- 用户真实姓名',
    `ALIAS` VARCHAR
(
    255
) COMMENT '「alias」- 用户昵称',
...
(
    255
) COMMENT '「mobile」- 用户登录手机',
    `EMAIL` VARCHAR
(
    255
) COMMENT '「email」- 用户登录EMAIL地址',
    `PASSWORD` VARCHAR
(
    255
) COMMENT '「password」- 用户登录密码',
...
(
    255
) COMMENT '「modelId」- 组所关联的模型identifier，用于描述',
    `MODEL_KEY` VARCHAR
(
    36
) COMMENT '「modelKey」- 组所关联的模型记录ID，用于描述哪一个Model中的记录',

    -- 特殊字段
    `CATEGORY` VARCHAR
(
    36
) COMMENT '「category」- 用户分类',
    -- ------------------------------ 公共字段 --------------------------------
    `SIGMA` VARCHAR
(
    128
