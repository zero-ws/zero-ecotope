...
class TwineQr {
    ...
    private static Future<JsonObject> normalizeG(final JsonObject criteriaJ) {
        // groupId,i / groups,i
        final String fieldId = ScAuthKey.F_GROUP_ID + ",i";
        ...
        if (criteriaJ.containsKey(fieldId)
            ...
            return DB.on(RUserGroupDao.class).<RUserGroup>fetchInAsync(ScAuthKey.F_GROUP_ID, value)
                .compose(relation -> {
    ...
    private static Future<JsonObject> normalizeR(final JsonObject criteriaJ) {
        // roleId,i / roles,i
        final String fieldId = ScAuthKey.F_ROLE_ID + ",i";
        final String fieldV = KName.ROLES + ",i";
        if (criteriaJ.containsKey(fieldId)
            || criteriaJ.containsKey(fieldV)) {
            final JsonArray value = valueBy(criteriaJ, fieldId, fieldV);
            return DB.on(RUserRoleDao.class).<RUserRole>fetchInAsync(ScAuthKey.F_ROLE_ID, value)
                .compose(relation -> {
