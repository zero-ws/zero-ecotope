...
(
    255
) COMMENT '「dataIndex」- 列名',
    `POSITION` INT COMMENT '「position」- 当前列的位置',
    `RENDER` VARCHAR
(
    64
) COMMENT '「render」- 使用的Render函数',
    /*
     * 每一列的详细配置信息
     */
    -- 强制宽度
...
(
    10
) COMMENT '「filterType」- $filter.type 支持列搜索时的搜索类型',
...
(
    128
) COMMENT '「formatFail」- $format时间格式专用',
