...
(
    36
) COMMENT '「vQuery」- 连接query到 grid -> query 节点',
    `V_SEARCH` VARCHAR
(
    36
) COMMENT '「vSearch」- 连接search到 grid -> options 节点',
    `V_TABLE` VARCHAR
(
    36
) COMMENT '「vTable」- 连接table到 grid -> table 节点',
    ...
    `CLASS_COMBINER` VARCHAR
(
    255
) COMMENT '「classCombiner」- 组装器',
