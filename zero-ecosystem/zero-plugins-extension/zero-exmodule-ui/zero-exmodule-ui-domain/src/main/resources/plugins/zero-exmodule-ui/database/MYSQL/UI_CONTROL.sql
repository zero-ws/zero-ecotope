...
(
    64
) COMMENT '「sign」- 控件使用的签名基本信息',
    `PAGE_ID` VARCHAR
(
    36
) COMMENT '「pageId」- 当前控件所在的页面ID',
    `TYPE` VARCHAR
(
    32
) COMMENT '「type」- 当前控件的类型：CONTAINER / COMPONENT / FORM / LIST，其中 FORM / LIST 需要访问子表',
...
(
    64
) COMMENT '「containerName」- 当前控件使用的容器名',
...
(
    64
) COMMENT '「componentName」- 当前控件使用的组件名',
    `COMPONENT_CONFIG` TEXT COMMENT '「componentConfig」- 当前控件使用的配置',
    `COMPONENT_DATA` VARCHAR
(
    255
) COMMENT '「componentData」- 当前控件使用的数据，使用表达式结构',
    /*
