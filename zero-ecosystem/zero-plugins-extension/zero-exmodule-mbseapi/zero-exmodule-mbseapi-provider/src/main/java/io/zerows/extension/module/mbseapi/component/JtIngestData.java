 ...
 */
public class JtIngestData implements JtIngest {
    ...
    private Envelop validate(final Envelop envelop, final JtUri uri) {
        ...
        Envelop error = JtIngestVerifier.validateRequired(this.getClass(), data, uri.paramRequired());
        /* Specification of mode */
        final ParamMode mode = uri.paramMode();
        if ((ParamMode.BODY == mode || ParamMode.DEFINE == mode) && null == error) {
            /* Body validation */
            error = JtIngestVerifier.validateContained(this.getClass(), data, uri.paramContained());
        }
        return error;
    }
}
