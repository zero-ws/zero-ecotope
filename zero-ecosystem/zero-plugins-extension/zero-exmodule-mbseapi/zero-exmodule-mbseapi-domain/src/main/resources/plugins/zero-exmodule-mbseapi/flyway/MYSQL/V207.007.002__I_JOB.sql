...
(
    255
) COMMENT '「comment」- 任务编码',

    -- 存储对应的数据
    `GROUP` VARCHAR
(
    64
) COMMENT '「group」- 任务组（按组查询），自由字符串',
...
(
    ...
    `DURATION_COMPONENT` VARCHAR
(
    255
) COMMENT '「durationComponent」对应复杂调度问题',
     ...
     */
    `INCOME_COMPONENT` VARCHAR
(
    255
) COMMENT '「incomeComponent」对应income，必须是JobIncome，@On -> input',
    `INCOME_ADDRESS` VARCHAR
(
    255
) COMMENT '「incomeAddress」对应incomeAddress，字符串，@On -> address',
    `OUTCOME_COMPONENT` VARCHAR
(
    255
) COMMENT '「outcomeComponent」对应outcome，必须是JobOutcome，@Off -> outcome',
    `OUTCOME_ADDRESS` VARCHAR
(
    255
) COMMENT '「outcomeAddress」对应outcomeAddress，字符串，@Off -> address',
