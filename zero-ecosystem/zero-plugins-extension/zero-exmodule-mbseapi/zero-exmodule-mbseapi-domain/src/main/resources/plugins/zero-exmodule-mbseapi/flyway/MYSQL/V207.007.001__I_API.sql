...
(
    255
) COMMENT '「uri」- 接口路径，安全路径位于 /api 之下',
    `METHOD` VARCHAR
(
    20
) COMMENT '「method」- 接口对应的HTTP方法',
...
(
    64
) COMMENT '「type」- 通信类型，ONE-WAY / REQUEST-RESPONSE / PUBLISH-SUBSCRIBE',
    `PARAM_MODE` VARCHAR
(
    20
) COMMENT '「paramMode」- 参数来源，QUERY / BODY / DEFINE / PATH',
...
(
    255
) COMMENT '「inPlug」- 参数请求流程中的插件',
    `IN_SCRIPT` VARCHAR
(
    255
) COMMENT '「inScript」- 【保留】参数请求流程中的脚本控制',
    /*
...
(
    255
) COMMENT '「outWriter」- 响应格式处理器',

    -- 服务层连接器
    `WORKER_TYPE` VARCHAR
(
    255
) COMMENT '「workerType」- Worker类型：JS / PLUG / STD',
    `WORKER_ADDRESS` VARCHAR
(
    255
) COMMENT '「workerAddress」- 请求发送地址',
    `WORKER_CONSUMER` VARCHAR
(
    255
) COMMENT '「workerConsumer」- 请求地址消费专用组件',
    `WORKER_CLASS` VARCHAR
(
    255
) COMMENT '「workerClass」- OX | PLUG专用，请求执行器对应的JavaClass名称',
    `WORKER_JS` VARCHAR
(
    255
) COMMENT '「workerJs」- JS 专用，JavaScript路径：running/workers/<app>/下的执行器',

    -- 响应格式处理
    `SERVICE_ID` VARCHAR
(
    36
) COMMENT '「serviceId」- 关联的服务ID',
