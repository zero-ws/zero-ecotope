...
class CoJAAAQr implements Co<JsonObject, JsonArray, JsonArray, JsonArray> {
    private transient final IxMod in;
    private transient final Co<JsonArray, JsonArray, JsonArray, JsonArray> record;

    CoJAAAQr(final IxMod in) {
        this.in = in;
        this.record = Pooled.CCT_CO.pick(() -> new CoAAAAData(in), CoAAAAData.class.getName() + in.cached());
