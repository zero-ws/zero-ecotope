...
import io.zerows.extension.module.finance.servicespec.EndSettleWStub;
 ...
 */
public class EndSettleWService implements EndSettleWStub {
    /**
     * 结算步骤
     * <pre><code>
     *      结算单 x 1
    ...
    public Future<List<FSettlement>> createAsync(final JsonArray body, final EmPay.Type type) {
        ...
        return Step.step01ST().scatter(body, type)
            .compose(settleRef::future)


            // 02. 根据结算单处理账单明细，此处外层调用 updateAsync 更新方法（防并发冲突）
            // (JsonArray, List<FSettlement>) -> List<FBillItem>
            .compose(inserted -> Step.step02BI().flatter(body, inserted))
            ...
            .compose(items -> Step.step04SI().flatter(Ux.toJson(items), settleRef.get()))
    ...
    public Future<List<FSettlement>> updateAsync(final JsonObject body, final User user) {
        ...
        return DB.on(FSettlementItemDao.class).<FSettlementItem>fetchInAsync(FmConstant.ID.SETTLEMENT_ID, settlementIds)
            ...
            .compose(keys -> {
                ...
                return Step.sync01ST().scatter(updatedData, user);
            })
            .compose(settlements -> {
                /*
                 * 还原过滤的最终结果，此处可以不关心完成
