 ...
 */
public class EndTransService implements EndTransStub {
    @Override
    ...
    public Future<FTrans> createBySettlement(final JsonObject data, final List<FSettlement> settlements) {
        ...
        // 1. 构造 FTrans
        return Step.step06T().flatter(data, settlements)
            // 2. 构造 FTransOf
            .compose(trans -> this.createTransItem(trans, data, params));
    }
    ...
    public Future<FTrans> createByDebt(final JsonObject data, final List<FDebt> debts) {
        ...
        // 1. 构造 FTrans
        return Step.step07T().flatter(data, debts)
            // 2. 构造 FTransOf
            .compose(trans -> this.createTransItem(trans, data, params));
    }
    ...
    private Future<FTrans> createTransItem(
        ...
        return Step.step06TO().scatter(params, trans)
            .compose(nil -> {
     ...
     */
    @Override
    public Future<List<DataTran>> fetchAsync(final Set<String> keys,
