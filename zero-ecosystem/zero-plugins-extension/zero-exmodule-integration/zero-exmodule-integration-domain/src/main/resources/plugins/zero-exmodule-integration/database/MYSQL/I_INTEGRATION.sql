...
(
    15
) COMMENT '「ipV4」- IP v4地址',
    `IP_V6` VARCHAR
(
    40
) COMMENT '「ipV6」- IP v6地址',
    `HOSTNAME` VARCHAR
(
    255
) COMMENT '「hostname」- 主机地址',

    `PORT` INT COMMENT '「port」- 端口号',
    `PROTOCOL` VARCHAR
(
    64
) COMMENT '「protocol」- 协议类型：HTTP, HTTPS, FTP',

    -- 特殊协议的安全相关信息
    `SECURE_PORT` INT COMMENT '「securePort」- 传输层安全接口',
    `SECURE_PROTOCOL` VARCHAR
(
    32
) COMMENT '「secureProtocol」- 传入层协议：TLS / SSL（邮件服务器需要）',

    `ENDPOINT` VARCHAR
(
    255
) COMMENT '「endpoint」- 端地址',
    `PATH` VARCHAR
(
    255
) COMMENT '「path」- 集成专用根路径',

    `OS_KEY` NVARCHAR
(
    1024
) COMMENT '「osKey」- 开源专用Key',
    `OS_SECRET` NVARCHAR
(
    1024
) COMMENT '「osSecret」- 开源专用Secret',
    `OS_AUTHORIZE` VARCHAR
(
    255
) COMMENT '「osAuthorize」- Authorize接口',
    `OS_TOKEN` VARCHAR
(
    255
) COMMENT '「osToken」- Token接口',
