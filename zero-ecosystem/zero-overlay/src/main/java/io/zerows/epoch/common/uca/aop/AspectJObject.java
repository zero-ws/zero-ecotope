...
class AspectJObject {
    ...
    <T> Function<T, Future<T>> wrapAop(
        ...
        final JsonObject config = UtBase.valueJObject(configuration);
        return input -> {
            // Default to input
            Future<T> stepFuture = Future.succeededFuture(input);
            if (Objects.nonNull(before)) {
                final JsonObject beforeConfig = UtBase.valueJObject(config, before.getClass().getName());
            ...
            if (Objects.nonNull(after)) {
                final JsonObject afterConfig = UtBase.valueJObject(config, after.getClass().getName());
