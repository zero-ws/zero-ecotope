package io.zerows.epoch.runtime;
...
/**
 * 核心系统环境变量接口，可直接实现
 * - HMacrocosm：宏观世界，核心空间
 *
 * @author <a href="http://www.origin-x.cn">Lang</a>
 */
public interface HMacrocosm {
    /*
     * 环境变量（配置KEY前缀说明）
     * - Z_DEV_           :       开发专用配置
     * - Z_CACHE_         :       缓存专用配置
     ...
     * 1 .dev.development           环境变量文件中的值优先（云发布）
    ...
    static String envContent() {
        ...
        Arrays.stream(VARS).filter(StrUtil::isNotEmpty).forEach(name -> {
