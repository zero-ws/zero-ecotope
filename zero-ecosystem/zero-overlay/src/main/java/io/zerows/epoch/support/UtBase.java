...
/**
 * High KMetadata Service 高阶元数据工具类
 * 1）动参版本
 * 2）对象版本
 ...
 */
public class UtBase extends _Value {
    /*
    ...
    public static String fromMessage(final String pattern, final Object... args) {
        return UFormat.fromMessage(pattern, args);
    }

    /**
     * 由于 {@link ResourceBundle} 本身是支持国际化资源绑定的，所以此处直接从 bundle 中提取
     * 资源键为 key 部分的字符串，然后使用 {@link UtBase#fromMessage} 进行格式化
     *
     * @param bundle 资源绑定对象
     * @param key    资源键
     * @param args   参数
    ...
    public static String fromResource(final ResourceBundle bundle,
        ...
        return UFormat.fromResource(bundle, key, args);
    }
