package io.zerows.metadata.boot;
 ...
 *   <li>🧱 <b>预配置阶段</b>：通过 {@link KConfigurer} 绑定 {@link HEnergy} 与命令行参数，抽取 {@link HConfig.HOn}（启动前置 On 组件）。</li>
 *   <li>🧰 <b>可插拔生命周期</b>：调用 {@link KConfigurer#preExecute(Object, HConfig)} 在容器就绪后执行第一步初始化。</li>
 ...
 *   <li>⚡ 从 {@link BootIo#energy(Class, String[])} 构建 {@link HEnergy}；创建并配置 {@link KConfigurer}。</li>
 ...
 *   <li>🛠️ 执行 {@link KConfigurer#preExecute(Object, HConfig)} 进行容器就绪后的首轮初始化。</li>
 ...
 *   <li>📝 通过 {@link LogAs.Boot} 输出启动组件相关日志（如发现的 {@link HLauncher} 实现类）。</li>
 * </ul>
 *
...
public class KLauncher<T> {
     ...
     *   <li>构造 {@link HEnergy} 并创建 {@link KConfigurer}，绑定命令行参数。</li>
     ...
     *   <li>若存在配置对象，调用 {@link KConfigurer#preExecute(Object, HConfig)} 执行容器就绪后的第一步初始化。</li>
