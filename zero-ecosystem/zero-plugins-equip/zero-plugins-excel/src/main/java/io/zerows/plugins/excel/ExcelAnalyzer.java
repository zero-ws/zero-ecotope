...
public class ExcelAnalyzer implements Serializable {
    ...
    public ExcelAnalyzer(final Sheet sheet) {
        this.sheet = sheet;
    }

    public ExcelAnalyzer on(final FormulaEvaluator evaluator) {
    ...
    public Set<ExTable> analyzed(final ExBound bound, final HMetaAtom metaAtom) {
        ...
        try {
            ...
            ExFn.itSheet(this.sheet, bound, (row, index) -> {
                final ExBound colBound = new ExBoundCol(row);
    ...
    private ExTable analyzed(final Row row, final Cell cell, final Integer limitation, final HMetaAtom metaAtom) {
        ...
        if (Objects.nonNull(metaAtom) && metaAtom.isComplex()) {
            in = new ExInComplex(this.sheet).bind(this.evaluator);
        } else {
            in = new ExInPure(this.sheet).bind(this.evaluator);
        }
        /*
         * Table processing
         */
