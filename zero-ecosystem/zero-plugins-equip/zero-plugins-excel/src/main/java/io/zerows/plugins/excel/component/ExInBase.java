...
import io.zerows.plugins.excel.util.ExOut;
 ...
 */
public abstract class ExInBase implements ExIn {
    protected transient Sheet sheet;
    protected transient FormulaEvaluator evaluator;

    public ExInBase(final Sheet sheet) {
        this.sheet = sheet;
    }
    ...
    protected Function<Cell, Object> formulaFn(final Cell dataCell, final Class<?> type) {
        ...
        if (CellType.FORMULA == cellType) {
            fun = ExValueFn.FN_CELL.get(CellType.STRING);
        } else {
            if (Objects.nonNull(type)) {
                final CellType switchedType = ExOut.type(type);
                if (Objects.isNull(switchedType)) {
                    fun = ExValueFn.FN_CELL.get(cellType);
                } else {
                    fun = ExValueFn.FN_CELL.get(switchedType);
                }
            } else {
                fun = ExValueFn.FN_CELL.get(cellType);
            }
