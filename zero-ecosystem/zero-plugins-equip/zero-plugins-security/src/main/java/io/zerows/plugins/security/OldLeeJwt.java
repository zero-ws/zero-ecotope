 ...
 */
class OldLeeJwt extends AbstractOldLee {
    ...
    public JWTAuth providerInternal(final Vertx vertx, final SecurityMeta config) {
        // Options
        final SecurityConfig securityConfig = null; // config.item();
        return this.provider(vertx, securityConfig);
    }

    private JWTAuth provider(final Vertx vertx, final SecurityConfig securityConfig) {
        final JWTAuthOptions options = new JWTAuthOptions(securityConfig.options());
