package io.zerows.boot.extension.util;
...
final class OxTo {
    ...
    static JsonObject toEdge(final JsonObject edgeData) {
        ...
        edge.put(OxConstant.Relation.SOURCE_PREFIX, edgeData.getValue(OxConstant.Relation.SOURCE_JOINED));
        edge.put(OxConstant.Relation.TARGET_PREFIX, edgeData.getValue(OxConstant.Relation.TARGET_JOINED));
    ...
    static <T> JsonArray toLinker(final T source) {
        final JsonArray links = (JsonArray) Ut.itJson(source, json -> {
            final JsonArray normalized = new JsonArray();
            normalized.addAll(Ut.valueJArray(json.getJsonArray(OxConstant.RELATION_DOWN)));
            normalized.addAll(Ut.valueJArray(json.getJsonArray(OxConstant.RELATION_UP)));
            return (T) normalized;
        });
