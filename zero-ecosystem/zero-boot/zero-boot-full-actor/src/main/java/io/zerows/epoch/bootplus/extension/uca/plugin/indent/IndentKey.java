package io.zerows.epoch.bootplus.extension.uca.plugin.indent;
...
public class IndentKey implements Identifier {
    ...
    private String key(final JsonObject data, final JsonObject config) {
        ...
        String keyField = config.getString(OxConstant.Field.DIM_3, OxConstant.Field.CATEGORY_THIRD);
        String key = data.getString(keyField);
        if (Ut.isNil(key)) {


            // Seeking by category second ( dim2 field )
            keyField = config.getString(OxConstant.Field.DIM_2, OxConstant.Field.CATEGORY_SECOND);
        ...
        if (Ut.isNil(key)) {
            ...
            keyField = config.getString(OxConstant.Field.DIM_1, OxConstant.Field.CATEGORY_FIRST);
            key = data.getString(keyField);
        }
        return key;
    }
