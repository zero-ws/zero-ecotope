...
public class QMenu {
    ...
    private static Future<JsonArray> fetchMenuTree(final List<String> types, final JsonArray required) {
        ...
        return DB.on(XMenuDao.class).searchJAsync(query).compose(pageData -> {
    ...
    /*
     * Menu fetching from database to InJson String
     */
    public static Future<JsonArray> menuFetch(final List<String> types, final boolean readable) {
    ...
    public static Future<ConcurrentMap<String, JsonArray>> menuInitialize(final Set<String> roles) {
        ...
        return Fx.combineM(menuFuture).compose(menuData -> {
    ...
    public static Future<Boolean> menuOutput(final ConcurrentMap<String, JsonArray> menuMap,
        ...
        menuMap.forEach((role, data) -> {
            final String outFile = VClassPath.init.permission.ui_menu.of(root, role); // DevDefault.pathMenu(root, role);
            LOG.Shell.info(QMenu.class, "[ Dev ] File output: {0}", outFile);
    ...
    private static JsonArray buildRequired(final String role) {
        final JsonArray required = Ut.ioJArray(VClassPath.init.permission.ui_menu.role.of(role)); // DevDefault.pathMenu(role);
