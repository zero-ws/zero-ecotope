...
import io.zerows.boot.test.LauncherTest;
import io.zerows.boot.test.PartyA;
...
import io.zerows.extension.module.mbsecore.component.file.ExcelReader;
...
public class QModeller {
    private static PartyA REFERENCE;
    ...
    public void preprocess(final Actuator actuator) {
        this.okA().onComplete(handler -> {
            ...
            final PartyA partyA = handler.result();
    ...
    private Future<PartyA> okA() {
        if (Objects.isNull(REFERENCE)) {
            ...
            return LauncherTest.of(container, environment).compose(okA -> {
                REFERENCE = okA;
