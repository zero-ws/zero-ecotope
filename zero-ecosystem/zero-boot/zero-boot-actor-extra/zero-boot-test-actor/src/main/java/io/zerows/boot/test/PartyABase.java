 ...
 */
public abstract class PartyABase implements PartyA {
    ...
    private final ConcurrentMap<String, PartyB> vendors = new ConcurrentHashMap<>();

    protected PartyABase(final Environment environment) {
        ...
        globalRef.vendors().forEach(name -> {
            ...
            this.vendors.put(name, PartyB.of(this, integration));
        });
        {
            // 检查环境是否启动完成
