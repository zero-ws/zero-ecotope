...
class AspectConfig {
    ...
    void config(final Class<?> clazz, final JsonObject config) {
        Objects.requireNonNull(clazz);
        this.configMap.put(clazz, UtBase.isNil(config) ? new JsonObject() : config);
    }
    ...
    <T> String forkKey(final T input) {
        final Class<?> clazz = UtBase.valueC(this.forkJ, VPC.aop.plugin_fork.ROBIN);
        final JsonObject config = UtBase.valueJObject(this.forkJ, VPC.aop.plugin_fork.CONFIG);
        if (Objects.nonNull(clazz)) {
            // 配置了 robin
            final HReturn.HString<T> robin = UtBase.singleton(clazz);
            return robin.execute(input, config);
        } else {
            // 未配置组件，暂时只考虑 type = FIELD 的情况
            final String by = UtBase.valueString(config, VPC.aop.plugin_fork.config.BY);
            Objects.requireNonNull(by);
            if (input instanceof JsonObject) {
                final JsonObject inputJ = (JsonObject) input;
                return UtBase.valueString(inputJ, by);
            } else if (input instanceof JsonArray) {
                final JsonArray inputA = (JsonArray) input;
                return UtBase.valueString(inputA, by);
            } else {
    ...
    private void bind(final Collection<Class<?>> clazzSet, final JsonObject configJ) {
        clazzSet.forEach(clazz -> {
            if (configJ.containsKey(clazz.getName())) {
                final JsonObject componentConfig = UtBase.valueJObject(configJ, clazz.getName());
