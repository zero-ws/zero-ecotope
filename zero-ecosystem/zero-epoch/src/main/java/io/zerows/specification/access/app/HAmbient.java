...
/**
 * <p>
 * 应用容器入口，针对不同的应用提供不同的 {@link HArk} 结构
 * <pre><code>
 *     1. 多租户 / 多应用    N : N
 *     2. 单租户 / 多应用    1 : N
 *     3. 单租户 / 单应用    1 : 1
...
public interface HAmbient {
     ...
     * <pre><code>
     *     - 若是单机运行部署，那么在 {@link HDeployment} 计划中可
     *       直接提取 ( APP, TENANT ) 信息。
     *     - 若是平台运行部署，则可以返回所有信息。
     * </code></pre>
     *
     ...
     * <pre><code>
     *     1. 第一部分为当前 {@link HArk} 对应的 {@link HBelong} 中的
     *        owner()，此值全局唯一
     ...
     * 注册应用容器信息，注册容器的基本流程
     * <pre><code>
     *     1. 应用容器中的 {@link HBelong} 会隶属不同层级的
     *        基本信息，层级判断基于它的子类
     *     2. 内置的应用程序池只有在注册之后尺寸 > 1 时执行抽象层判断，否则注册就是单纯的类型
     *     3. 如果程序池中只有一个 {@link HArk}，则 mode = {@link EmApp.Mode#CUBE}，默认值
     *        若程序池中出现了超过 1 个应用，此处的 mode 值依赖 {@link HBelong}
     *        的实际类型
     *        - {@link HFrontier}, mode = {@link EmApp.Mode#FRONTIER}
     *        - {@link HGalaxy}, mode = {@link EmApp.Mode#GALAXY}
     *        - {@link HSpace}, mode = {@link EmApp.Mode#SPACE}
     * </code></pre>
     *
     * @param ark 应用容器
     *
