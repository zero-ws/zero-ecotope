...
final class VsJsonArray extends AbstractSame {
    ...
    public boolean isAnd(final Object valueOld, final Object valueNew) {
        ...
        if (UtBase.isJArray(valueOStr) && UtBase.isJArray(valueNStr)) {
            ...
            if (arrayNew.size() != arrayOld.size()) {
                ...
                if (Objects.isNull(this.fieldType)) {
                    ...
                    return UtBase.itJArray(arrayOld).allMatch(jsonOld -> UtBase.itJArray(arrayNew).anyMatch(jsonNew -> {
                        final JsonObject checkedNew = UtBase.elementSubset(jsonNew, diffSet);
                        final JsonObject checkedOld = UtBase.elementSubset(jsonOld, diffSet);
