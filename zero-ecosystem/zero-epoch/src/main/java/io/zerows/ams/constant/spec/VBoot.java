...
import io.zerows.specification.configuration.boot.HInstall;
 ...
 * 启动器核心目录设计
 * <p>
 * 内置配置对象 {@link HStarter}，统一配置接口
 * <pre><code>
 * boot:                # 启动器主配置
 *     launcher:        # 「路径」主容器配置 {@link HLauncher}
 *                           # 主容器1：OSGI容器 -> Jetty Bundle
 *                           # 主容器2：OSGI容器 -> Rx Vertx
 *                           # 主容器3：VertxApplication（单机版Zero标准）
 *                           # 主容器4：AeonApplication（云端启动器）
 *     component:       # {@link BootIo} 中提供并执行初始化
 *        on:           # 配置内置于 {@link HConfig.HOn}
 *        off:          # 配置内置于 {@link HConfig.HOff}
 *        run:          # 配置内置于 {@link HConfig.HRun}
 *     config:          # 实现层所需启动的基础配置，提供路径格式
 *                           # - OSGI容器配置路径
 *                           # - Web容器配置外置路径
 *     rad:             # 配置内置于 {@link HRAD}
 *     connect:         # 空间连接专用配置
 *        frontier:     # 配置内置于 {@link HFrontier}
 *        galaxy:       # 配置内置于 {@link HGalaxy}
 *        space:        # 配置内置于 {@link HSpace}
 * </code></pre>
 * 所有组件对应的配置使用 {@link BootIo} 的 SPI提供，此 SPI 会用于构造所有启动节点的核心
 * 区域组件，核心接口改造流程处理：
 * <pre><code>
 * 1. {@link HLauncher} 启动容器
 *    1.1. 内置初始化 {@link HStarter} 组装所有配置数据
 *         提取上述节点所有配置信息，内置调用 {@link BootIo} 提取不同的配置组件
 *         开启启动组件表，而配置数据则位于不同节点
 *    1.2. 抽取配置信息以初始化配置对象（配置对象只有一套）
 *    1.3. 根据最终结果，构造核心启动组件，然后触发 {@link HLauncher}
 * 2. 组件启动
 *    -- {@link HLauncher}              start
 *       -- {@link HConfig.HOn}               configure 启动配置初始化
 ...
 *    - run：用于更新流程
 *    - rad：用于开发中心
 *    - connect：根据不同的方式获取 {@link HAccount}，连接安全中心实现
 *               不同空间模式的整体连接流程
 * </code></pre>
 *
 * </p>
