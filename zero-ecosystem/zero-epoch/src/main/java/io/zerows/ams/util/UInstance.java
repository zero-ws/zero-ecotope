...
class UInstance {
    ...
    static <T> T instance(final Class<?> clazz, final Object... params) {
        ...
        return Fn.jvmOr(() -> construct(clazz, params));
    }
    ...
    static Class<?> clazz(final String name, final Class<?> instanceCls, final ClassLoader loader) {
        if (TIs.isNil(name)) {
             ...
             */
            final ConcurrentMap<String, Class<?>> cData = CACHE.CC_CLASSES.get();
            Class<?> clazz = cData.get(name);
            if (Objects.isNull(clazz)) {
                final Logger LOGGER = LoggerFactory.getLogger(UInstance.class);
                /* 优先从传入的类加载器中加载 */
                try {
    ...
    private static <T> T construct(final Class<?> clazz,
        ...
        if (BUILD_IN.containsKey(clazz)) {
            ...
            return Fn.jvmOr(() -> constructor.newInstance(params));
        } else {
            // 填充数据后递归
            final Constructor<?>[] constructors = clazz.getDeclaredConstructors();
    ...
    static <T> T get(final Object instance,
        ...
        if (Objects.nonNull(field)) {
            ...
            final Object result = Fn.jvmOr(() -> field.get(instance));
            if (null != result) {
                return (T) result;
            } else {
                return null;
