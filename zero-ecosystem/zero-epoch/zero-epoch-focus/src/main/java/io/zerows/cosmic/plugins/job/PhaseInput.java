    ...
    Future<Envelop> inputAsync(final Mission mission) {
        ...
        if (Ut.isNil(address)) {
            ...
            PhaseHelper.logOnce(mission, () ->
                log.debug("[ ZERO ] ( Job {} ) 1. 输入 JsonObject 类型的数据。", mission.getCode()));
             ...
             */
            log.debug("[ ZERO ] ( Job ) inputAsync 事件总线 EventBus 启用，地址：{}", address);
            final Promise<Envelop> input = Promise.promise();
            final EventBus eventBus = this.vertx.eventBus();
            eventBus.<Envelop>consumer(address, handler -> {

                PhaseHelper.logOnce(mission, () ->
                    log.debug("[ ZERO ] ( Job {} ) 1. 从事件总线地址 {} 上接收到输入数据。", mission.getCode(), address));
    ...
    Future<Envelop> incomeAsync(final Envelop envelop, final Mission mission) {
        ...
        if (envelop.valid()) {
            if (Objects.isNull(income)) {
                 ...
                 */
                PhaseHelper.logOnce(mission, () ->
                    log.debug("[ ZERO ] ( Job {} ) 2. 无需 JobIncome 定义，直接跳过。", mission.getCode()));
                 ...
                 */
                log.debug("[ ZERO ] ( Job {} ) 2. 启用 JobIncome 组件 {} 。", mission.getCode(), income.getClass().getName());
                /*
                 ...
                 */
                PhaseHelper.logOnce(mission, () ->
                    log.debug("[ ZERO ] ( Job {} ) 2. 开始执行 JobIncome 组件 {} 。", mission.getCode(), income.getClass().getName()));
