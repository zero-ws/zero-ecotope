...
public interface OldLee {
     ...
     */
    String encode(JsonObject data, SecurityConfig config);

    JsonObject decode(String token, SecurityConfig config);

    // ------------------ New Interface for Extension ----------------------
    /*
     * 4.  AuthenticationProvider wrapped by ( Default + Extension )
     */
    AuthenticationProvider provider(Vertx vertx, SecurityMeta config);
}
