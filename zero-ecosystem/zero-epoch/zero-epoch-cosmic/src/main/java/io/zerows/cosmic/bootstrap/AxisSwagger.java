    ...
    public void mount(final RunServer server, final HBundle bundle) {
        ...
        if (Objects.isNull(factory)) {
            ...
            if (LOG_FACTORY_NULL.compareAndSet(false, true)) {
                log.debug("[ ZERO ] ( Swagger ) ⚠️ SPI 组件 AxisSwaggerFactory 为 null，Swagger 功能禁用！");
            }
            return;
        }
        if (factory.isEnabled(bundle, server)) {
            if (LOG_ENABLED.compareAndSet(false, true)) {
                log.info("[ ZERO ] ( Swagger ) Swagger 功能启动成功！,登录地址：https://{}/docs/#/", server.name());
            }
        } else {
            if (LOG_DISABLED.compareAndSet(false, true)) {
                log.debug("[ ZERO ] ( Swagger ) 功能被配置禁用，请检查配置或联系管理员！");
            }
        }
    }
}
