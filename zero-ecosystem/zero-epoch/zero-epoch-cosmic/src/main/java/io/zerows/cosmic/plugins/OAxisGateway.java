 ...
 *        每个 Factory 都可以根据双环境（OSGI 和普通环境）来提取对应的 {@link Axis} 路由挂载器。
 *     2. 设置端
 *        每一种特殊的 {@link Axis} 会带有特定的 Factory 来获取，这些 Factory 在非 OSGI 环境中直接走 SPI 来获取，而 OSGI 环境中
 *        走 OSGI Service 来提取，一旦拿到引用之后就可以得到对应的 {@link Axis} 路由挂载器了。
 * </code></pre>
 *
