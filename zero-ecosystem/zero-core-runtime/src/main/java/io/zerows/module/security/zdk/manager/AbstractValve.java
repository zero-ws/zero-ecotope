...
public abstract class AbstractValve implements HValve {
    ...
    public Future<JsonObject> configure(final JsonObject input) {
        ...
        RFn.out(EmSecure.ScIn.NONE == permit.source(), _409UiSourceNoneException.class, this.getClass(), permit.code());
        /*
         * 管理界面分为两个区域：
         * DM - 维度管理区域，UI - 数据管理区域
        ...
        if (EmSecure.ScDim.NONE == permit.shape()) {
            ...
            RFn.out(EmSecure.ActPhase.EAGER != phase, _409UiPhaseEagerException.class, this.getClass(), phase);
            // 只处理 Ui 部分，不处理其他部分
